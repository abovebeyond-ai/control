...
| Elixir's `major-upgrade` hand | the model (Anthropic, hosted) | one edit in Elixir's clone per attempt, Read/Edit/Grep/Glob, no shell | the model's identifier, the digest of its instructions and the attempt are in the working and in the bill of materials beside the record | the gateway pushes only what the working argues and the digest binds; the project's CI on the branch judges; a person merges |
