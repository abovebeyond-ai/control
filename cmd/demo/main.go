...
func main() {
	...
	good := material(1)
	bad := material(0)
	fmt.Println(g.Submit(policy.Action{Kind: "workflow.dispatch", Resource: "x/y", Params: map[string]any{"workflow": "elixir-fix.yml", "ref": "main", "packages": 1}}, "did:webvh:QmTest:example.org", map[string]any{"project": "demo"}, good).Verdict)
	fmt.Println(g.Submit(policy.Action{Kind: "pull.open", Resource: "x/y", Params: map[string]any{"branch": "elixir/security-2026-09-07", "base": "main"}}, "did:webvh:QmTest:example.org", nil, nil).Verdict)
	fmt.Println(g.Submit(policy.Action{Kind: "pull.merge", Resource: "x/y"}, "did:webvh:QmTest:example.org", nil, nil).Verdict)
	fmt.Println(g.Submit(policy.Action{Kind: "workflow.dispatch", Resource: "x/y", Params: map[string]any{"workflow": "w", "ref": "main"}}, "did:webvh:QmTest:example.org", nil, bad).Verdict)
	cp, err := g.Checkpoint()
	fail(err)
